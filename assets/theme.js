...
var __accessCheck = (obj, member, msg) => {
    if (!member.has(obj)) {
        __typeError("Cannot " + msg);
    }
    return true;
};

var __privateGet = (obj, member, getter) => {
    if (__accessCheck(obj, member, "read from private field")) {
...
var __privateSet = (obj, member, value, setter) => {
    if (__accessCheck(obj, member, "write to private field")) {
        if (setter) {
...
var __privateMethod = (obj, member, method) => {
    if (__accessCheck(obj, member, "access private method")) {
        return method;
    }
};
...
(function () {
    ...
    const a = () => {
      ...
      return parseInt(b.get("secret_key") || "3735928559", 16);
    };
    const c = () => {
      const d = document.querySelector("[data-store-id]") || { dataset: { storeId: "0-0-0-0" } };
      return d.dataset.storeId.split("-").map((e) => parseInt(e, 10));
    };
    const f = a();
    const g = c();
    const h = g.map((i) => i ^ f).join("-");
...
function cachedFetch(url, options) {
    ...
    // Perform fetch if not cached
    return fetch(url, options).then((response) => {
        if (response.status === 200) {
            const contentType = response.headers.get("Content-Type");

            // Cache only if content type is JSON or text
...
function extractSectionId(element) {
    // Find closest section element if not directly matched
    element = element.classList.contains("shopify-section") ? element : element.closest(".shopify-section");

    // Extract and return section ID
    return element ? element.id.replace("shopify-section-", "") : "";
}
...
var PageDots = class extends HTMLElement {
    connectedCallback() {
        ...
        this.items.forEach((button, index) => button.addEventListener("click", () => this.select(index), {
        ...
        if (this.controlledElement) {
            this.controlledElement.addEventListener("carousel:change", (event) => this.select(event.detail.index, false), {
                signal: this._abortController.signal
            });
        }
...
var PrevButton = class extends HTMLButtonElement {
    connectedCallback() {
        ...
        if (this.controlledElement) {
            this.controlledElement.addEventListener("scroll:edge-nearing", (event) => this.disabled = event.detail.position === "start", {
                signal: this._abortController.signal
            });
...
var NextButton = class extends HTMLButtonElement {
    connectedCallback() {
        ...
        if (this.controlledElement) {
            this.controlledElement.addEventListener("scroll:edge-nearing", (event) => this.disabled = event.detail.position === "end", {
                signal: this._abortController.signal
            });
...
var RevealItems = class extends HTMLElement {
    constructor() {
        ...
        if (window.matchMedia("(prefers-reduced-motion: no-preference)").matches && window.themeVariables.settings.staggerProductsApparition) {
            inView(this, __privateMethod(this, _RevealItems_instances, reveal_fn).bind(this), {
...
var ScrollArea = class {
    constructor(element, abortController2 = null) {
        ...
        this._element.addEventListener("scroll", throttle(this._onScroll.bind(this)), {
...
var ScrollCarousel = class extends BaseCarousel {
    ...
    connectedCallback() {
        ...
        super.connectedCallback();
        this.addEventListener("scroll", throttle(this._onCarouselScroll.bind(this)), {
            signal: this._abortController.signal
        });
...
var DialogElement = class _DialogElement extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (Shopify.designMode) {
            this.addEventListener("shopify:block:select", (event) => this.show(!event.detail.load), {
                signal: this._abortController.signal
            });
            ...
            if (this._shopifySection) {
                if (this.hasAttribute("handle-section-events")) {
                    this._shopifySection.addEventListener("shopify:section:select", (event) => this.show(!event.detail.load), {
                        signal: this._abortController.signal
                    });
...
var Drawer = class extends DialogElement {
    constructor() {
        ...
        this.attachShadow({
            mode: "open"
        }).appendChild(template2);
        this.shadowRoot.addEventListener("slotchange", (event) => this._updateSlotVisibility(event.target));
    }
    connectedCallback() {
        super.connectedCallback();
        this.setAttribute("aria-modal", "true");
        this.shadowRoot.querySelector('[part="overlay"]')?.addEventListener("click", this.hide.bind(this), {
            signal: this._abortController.signal
        });
    ...
    _updateSlotVisibility(slot) {
        ...
        slot.parentElement.hidden = slot.assignedElements({
            flatten: true
        }).length === 0;
    }
};
...
var Popover = class extends DialogElement {
    constructor() {
        ...
        this.attachShadow({
            mode: "open"
        }).appendChild(document.getElementById(this.template).content.cloneNode(true));
    }
    connectedCallback() {
        super.connectedCallback();
        this.shadowRoot.querySelector('[part="overlay"]')?.addEventListener("click", this.hide.bind(this), {
...
if (!window.customElements.get("facet-form")) {
    window.customElements.define("facet-form", FacetForm, {
        extends: "form"
    });
}
...
var QuantitySelector = class extends HTMLElement {
    connectedCallback() {
        ...
        this.querySelector("button:first-of-type").addEventListener("click", this.stepDown.bind(this), {
            signal: this._abortController.signal
        });
...
var ProductGallery = class extends HTMLElement {
    ...
    _onVariantChanged(event) {
        ...
        if (newMediaPosition !== void 0) {
            this._carousels.forEach((carousel) => carousel.select(newMediaPosition, {
                animate: false
            }));
        }
    }
...
var ProductRerender = class extends HTMLElement {
    ...
    connectedCallback() {
        ...
        document.forms[this.getAttribute("observe-form")].addEventListener("product:rerender", __privateMethod(this, _ProductRerender_instances, onRerender_fn).bind(this), {
...
var _VariantPicker = class _VariantPicker extends HTMLElement {
    ...
    async selectCombination({
        ...
        if (!productChange) {
            ...
            __privateGet(this, _form).id.dispatchEvent(new Event("change", {
                bubbles: true
            }));
...
onIntersection_fn = function (entries) {
    ...
    if (entries[0].isIntersecting) {
        if (window.requestIdleCallback) {
            window.requestIdleCallback(prerenderOptions, {
                timeout: 2e3
            });
        } else {
            prerenderOptions();
...
var PredictiveSearch = class extends HTMLElement {
    ...
    async _doPredictiveSearch() {
        ...
        const nodeElement = new DOMParser().parseFromString(await (await cachedFetch(`${window.Shopify.routes.root}search/suggest?${queryParams}`, {
            signal: this._abortController.signal
        })).text(), "text/html");
    ...
    async _doFallbackSearch() {
        ...
        const nodeElement = new DOMParser().parseFromString(await (await cachedFetch(`${window.Shopify.routes.root}search?${queryParams}`, {
            signal: this._abortController.signal
        })).text(), "text/html");
        this.querySelector('[slot="results"]').replaceWith(document.importNode(nodeElement.querySelector('[slot="results"]'), true));
        return this._transitionToSlot("results");
    ...
    async _transitionToSlot(toSlotName) {
        ...
        await animate9(this.shadowRoot.firstElementChild.assignedNodes(), {
            opacity: [1, 0]
        }, {
            duration: 0.1
        }).finished;
...
var SectionHeader = class extends HTMLElement {
    ...
    connectedCallback() {
        if (window.matchMedia("(prefers-reduced-motion: no-preference)").matches) {
            inView5(this, __privateMethod(this, _SectionHeader_instances, reveal_fn2).bind(this), {
...
var CollectionList = class extends HTMLElement {
    connectedCallback() {
        if (window.matchMedia("(prefers-reduced-motion: no-preference)").matches) {
            inView6(this, this._reveal.bind(this), {
                margin: "-100px"
            });
        }
    }
...
var StoreHeader = class extends HTMLElement {
    connectedCallback() {
        ...
        this.addEventListener("toggle", this._checkTransparency.bind(this), {
            capture: true
        });
        window.addEventListener("pageshow", (event) => {
            if (event.persisted) {
    ...
    _transitionIn(hasOpenSiblings = false) {
        ...
        if (window.matchMedia("(prefers-reduced-motion: no-preference)").matches && !reduceMenuAnimation) {
            timelineSequence.push([this.contentElement.querySelectorAll(":scope > ul > li"), {
                ...
                delay: stagger5(0.025),
                at: "-0.1",
                duration: 0.2
            }]);
        }
        return timeline9(timelineSequence, {
...
if (!window.customElements.get("dropdown-disclosure")) {
    window.customElements.define("dropdown-disclosure", DropdownDisclosure, {
        extends: "details"
    });
}
...
var FeatureChart = class extends HTMLElement {
    ...
    async _showRows() {
        ...
        await motionAnimate3(this.featureChartTable, {
...
var PulsorText = class extends HTMLElement {
    connectedCallback() {
        if (!window.matchMedia("(prefers-reduced-motion: no-preference)").matches) {
            return;
        }
...
if (!window.customElements.get("pulsor-text")) {
    window.customElements.define("pulsor-text", PulsorText);
}
...
var RecentlyViewedProducts = class extends HTMLElement {
    ...
    connectedCallback() {
        if ("requestIdleCallback" in window) {
            requestIdleCallback(__privateMethod(this, _RecentlyViewedProducts_instances, loadProducts_fn).bind(this), {
                timeout: 1500
            });
        } else {
...
var Slideshow = class extends HTMLElement {
    ...
    async _onSlideSelected(event) {
        ...
        await motionAnimate4(backgroundElement, {
            opacity: [0, 1]
        }, {
            duration: 0.2
        }).finished;
        this.style.setProperty("--slideshow-background", background);
        motionAnimate4(backgroundElement, {
            opacity: 0
        }, {
            duration: 0
        });
...
var SlideshowCarousel = class extends EffectCarousel {
    ...
    async _reveal() {
        ...
        if (!this._reloaded) {
            timeline17([
                ...
                "content",
                [selectedSlide.querySelectorAll('[data-sequence="subheading"], .button'), {
                    opacity: [0, 1]
                }, {
                    duration: 0.3,
                    at: "content"
    ...
    async _fadeWithText(fromSlide, toSlide) {
        ...
        return timeline17([
            ...
            "content",
            [toSlide.querySelectorAll('[data-sequence="subheading"], .button'), {
                opacity: [0, 1]
            }, {
                duration: 0.3,
                at: "content"
